...
async def camera_loop(
    ...
    while True:
        ...
        # ---- Grab both frames atomically (one lock) ----
        ...
        if detect_frame is None:
            ...
            no_cam_msg = json.dumps({
                "type": "frame", "face_detected": False, "face_count": 0,
        ...
        # ---- Attention detection: pass BOTH frames ----
        ...
        if not isinstance(detection, dict) or "attention_ok" not in detection:
            logger.warning("detector.process() unexpected format — treating as face present.")
            detection = {
                "face_detected": False, "face_count": 0,
        ...
        # ---- JPEG preview (reuse main_frame already in hand) ----
        _preview_counter += 1
